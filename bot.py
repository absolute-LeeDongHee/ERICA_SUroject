...
async def on_message(message):
	...
	if message.content.startswith("!명령어"):
		...
		embed.add_field(name='!낭비 닉네임',value = "롤을 안 했더라면 할 수 있었던 일을 알려줍니다.", inline=False)
	...
	if message.content.startswith("!낭비"):
		channel = message.channel
		Name = message.content[4:]
		fact = []
		url = 'https://kr.wol.gg/stats/kr/' + Name + '/'
		...
		soup = BeautifulSoup(html, 'html.parser')
		# 소환사 이름 크롤링
		summonername4 = soup.find(['h1'])
		for i in soup.find_all('div', 'fact center'):
			fact.append(i)
		if summonername4 != "":
